...
import { Profile } from "@/app/services/resumeService";

export default async function Dashboard() {
    const resumeUrl = `${process.env.NEXT_PUBLIC_BASE_URL}/resume.pdf`;
    const url = `${process.env.NEXT_PUBLIC_BASE_URL}/api/resume`;

    const profile: Profile | undefined = await fetch(url, {
        method: 'GET'
    }).then(response => response.json())
    .then(profile => {
        return profile as Profile;
    }).catch(error => {
        console.error("Error loading profile data from API: " + (error?.message || error));
        return undefined;
    });
